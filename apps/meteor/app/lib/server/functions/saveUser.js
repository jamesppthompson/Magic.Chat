...
function validateUserData(userId, userData) {
  ...
  if (
    userData._id &&
    userId !== userData._id &&
    !hasPermission(userId, "edit-other-user-info")
  ) {
    throw new Meteor.Error(
  ...
  if (
    userData.roles &&
    _.difference(userData.roles, existingRoles).length > 0
  ) {
    throw new Meteor.Error(
      "error-action-not-allowed",
      "The field Roles consist invalid role id",
      {
        method: "insertOrUpdateUser",
        action: "Assign_role",
      }
  ...
  if (
    userData.roles &&
    _.indexOf(userData.roles, "admin") >= 0 &&
    !hasPermission(userId, "assign-admin-role")
  ) {
    throw new Meteor.Error(
      "error-action-not-allowed",
  ...
  if (userData.username && !nameValidation.test(userData.username)) {
    throw new Meteor.Error(
      ...
      `${_.escape(userData.username)} is not a valid username`,
      {
        method: "insertOrUpdateUser",
  ...
  if (!userData._id) {
    if (!checkUsernameAvailability(userData.username)) {
      throw new Meteor.Error(
        ...
        `${_.escape(userData.username)} is already in use :(`,
        {
          method: "insertOrUpdateUser",
    ...
    if (userData.email && !checkEmailAvailability(userData.email)) {
      throw new Meteor.Error(
        "error-field-unavailable",
        `${_.escape(userData.email)} is already in use :(`,
        {
          method: "insertOrUpdateUser",
...
export function validateUserEditing(userId, userData) {
  ...
  const canEditOtherUserPassword = hasPermission(
    userId,
  ...
  const isEditingUserRoles = (previousRoles, newRoles) =>
    typeof newRoles !== "undefined" &&
    !_.isEqual(_.sortBy(previousRoles), _.sortBy(newRoles));
  const isEditingField = (previousValue, newValue) =>
    typeof newValue !== "undefined" && newValue !== previousValue;

  if (
    isEditingUserRoles(user.roles, userData.roles) &&
    !hasPermission(userId, "assign-roles")
  ) {
    throw new Meteor.Error(
      "error-action-not-allowed",
  ...
  if (
    !settings.get("Accounts_AllowUserProfileChange") &&
    !canEditOtherUserInfo &&
    !canEditOtherUserPassword
  ) {
    throw new Meteor.Error(
      "error-action-not-allowed",
      "Edit user profile is not allowed",
      {
        method: "insertOrUpdateUser",
        action: "Update_user",
      }
  ...
  if (
    ...
    throw new Meteor.Error(
      "error-action-not-allowed",
      "Edit user status is not allowed",
      {
        method: "insertOrUpdateUser",
        action: "Update_user",
      }
  ...
  if (
    ...
    throw new Meteor.Error(
      "error-action-not-allowed",
      "Edit user real name is not allowed",
      {
        method: "insertOrUpdateUser",
        action: "Update_user",
      }
  ...
  if (
    ...
    throw new Meteor.Error(
      "error-action-not-allowed",
      "Edit user email is not allowed",
      {
        method: "insertOrUpdateUser",
        action: "Update_user",
      }
  ...
  if (
    ...
    (!canEditOtherUserPassword || editingMyself)
  ) {
    throw new Meteor.Error(
      "error-action-not-allowed",
      "Edit user password is not allowed",
      {
        method: "insertOrUpdateUser",
        action: "Update_user",
      }
...
const saveNewUser = function (userData, sendPassword) {
  ...
  if (userData.sendWelcomeEmail) {
    _sendUserEmail(
      settings.get("Accounts_UserAddedEmail_Subject"),
      html,
      userData
    );
  }
